...
from models import build_cascade_dn_detr
...
def get_args_parser():
    ...
    # Model parameters
    parser.add_argument('--modelname', '-m', type=str, required=True, choices=['cascade_dn_detr'])
    ...
    # cascade attention
    parser.add_argument('--cascade_attn', action='store_true',
                        help="box attention decoder")

    return parser


def build_model_main(args):
    if args.modelname.lower() == 'cascade_dn_detr':
        model, criterion, postprocessors = build_cascade_dn_detr(args)
    else:

...
class BlankMultiheadAttention(Module):
    ...
    def __init__(self, embed_dim, num_heads, dropout=0., bias=True, add_bias_kv=False, add_zero_attn=False, kdim=None, vdim=None):
        super(BlankMultiheadAttention, self).__init__()
    ...
    def __setstate__(self, state):
        # Support loading old BlankMultiheadAttention checkpoints generated by v1.1.0
        if '_qkv_same_embed_dim' not in state:
            state['_qkv_same_embed_dim'] = True

        super(BlankMultiheadAttention, self).__setstate__(state)
    ...
    Shape:
        ...
        if not self._qkv_same_embed_dim:
            return blank_multi_head_attention_forward(
...
def blank_multi_head_attention_forward(query: Tensor,
                                 key: Tensor,
                                 value: Tensor,
    ...
    if not torch.jit.is_scripting():
        ...
        if any([type(t) is not Tensor for t in tens_ops]) and has_torch_function(tens_ops):
            return handle_torch_function(
                blank_multi_head_attention_forward, tens_ops, query, key, value,

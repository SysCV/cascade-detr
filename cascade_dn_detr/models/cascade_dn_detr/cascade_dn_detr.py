...
from .hook_refpoint import add_half_hw, add_xy_plus_0p1, offset_0p01
...
class Cascade_DNDetr(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
        ...
        """
        '''
                query_dim=args.pdetr3_query_dim, #4
                random_refpoints_xy=args.pdetr3_random_refpoints_xy, #False
                fix_refpoints_hw=args.pdetr3_fix_refpoints_hw, #-1
                yolo_like_anchor_update=args.dabdetr_yolo_like_anchor_update, #dabdetr_yolo_like_anchor_update
        ...
        self.iou_embed = nn.Linear(hidden_dim, 1)
    ...
    def forward(self, samples: NestedTensor,dn_args=None):
        ...
        src_size = [[torch.ceil(target['size']/8).cpu(),torch.ceil(target['size']/16).cpu(),torch.ceil(target['size']/32).cpu(),torch.ceil(target['size']/64).cpu()] for target in dn_args[0]]
        orig_size = [target['orig_size'].cpu() for target in dn_args[0]]

        hs, reference = self.transformer(srcs, masks, input_query_bbox, poss, tgt=input_query_label,
                                        attn_mask=attn_mask, src_size=src_size, orig_size=orig_size)
        ...
        if os.environ.get('PDETR_SAVE_REF_POINTS', False):
            torch.save({
                'reference': reference,
                'outputs_coord': outputs_coord,
            },  os.environ.get('PDETR_SAVE_REF_POINTS'))

        outputs_class = self.class_embed(hs)
        outputs_iou = self.iou_embed(hs)
        outputs_class, outputs_coord, outputs_iou = dn_post_process(outputs_class, outputs_coord, outputs_iou, mask_dict)
        out = {'pred_logits': outputs_class[-1], 'pred_boxes': outputs_coord[-1], 'pred_logits_iou':outputs_iou[-1]}
        if self.aux_loss:
            out['aux_outputs'] = self._set_aux_loss(outputs_class, outputs_coord, outputs_iou)
...
class SetCriterion(nn.Module):
    ...
    def forward(self, outputs, targets,mask_dict=None,return_indices=False):
        ...
        if 'aux_outputs' in outputs:
            for idx in range(len(outputs['aux_outputs'])):
                aux_outputs=outputs['aux_outputs'][idx]
    ...
    def tgt_loss_labels(self, src_logits_, tgt_labels_, num_tgt, log=True):
    ...
    def convert_long(self, mask_dict):
        dict = {k: torch.as_tensor(v).long() for k, v in mask_dict.items()}
...
class PostProcess(nn.Module):
    ...
    @torch.no_grad()
    def forward(self, outputs, target_sizes, not_to_xyxy=False, scale_boxes=True, test=False, de_aug=False):
        ...
        prob = out_logits.sigmoid()     # 300 * 91
        ...
        if de_aug:
            ...
            boxes = boxes[:, :, [2, 1, 0, 3]] * torch.as_tensor([[[-1, 1, -1, 1]]]).cuda() + torch.as_tensor(
                [[[1, 0, 1, 0]]]).cuda()
...
def build_cascade_dn_detr(args):
    ...
    num_classes_dict = {'coco':91,"uvo":2,'cityscapes':9,'brain_tumor':4,'document_parts':3,'smoke':2,
                        'egohands':2,'plantdoc':31,'people_in_paintings':3,'bdd100k':11}
    try:
        num_classes = num_classes_dict[args.dataset_file]
    except:
        raise ValueError(f'num_classes of dataset {args.dataset_file} not registered')
    ...
    if args.masks:
        print("Not implement seg head now")
    ...
    label_scalar = 1.0
    box_scalar = 1.0
    weight_dict['tgt_loss_ce'] = 1 * label_scalar
    weight_dict['tgt_loss_bbox'] = args.bbox_loss_coef * box_scalar
    weight_dict['tgt_loss_giou'] = args.giou_loss_coef * box_scalar
    weight_dict['tgt_loss_iou'] = args.giou_loss_coef * box_scalar

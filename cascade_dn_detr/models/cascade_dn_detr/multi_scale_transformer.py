...
class MultiScaleTransformerDecoderLayer(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        for idx, (ki, vi, mask_pad_i, ca_module) in enumerate(zip(k_list, v_list, memory_key_padding_mask_list, self.cross_attn_list)):
            tgt2_i, weight_cross = ca_module(query=q,
                                            key=ki,
                                            value=vi, 
                                            key_padding_mask=mask_pad_i,
                                            attn_mask=memory_mask[idx])
            tgt2_i = self.ca_out_v_proj(tgt2_i) # nq, bs, d_model
            tgt2_list.append(tgt2_i)
            if idx == 0:
                weight_return = weight_cross    # For visualization attn_map

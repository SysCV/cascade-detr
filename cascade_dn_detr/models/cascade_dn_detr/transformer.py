...
def gen_memory_mask(reference_points,src_size,src_shape,num_heads):
    with torch.no_grad():
        mem_mask_list = []
        for level in range(len(src_shape)):
            mem_mask_shape = (reference_points.shape[1],reference_points.shape[0]) + tuple(src_shape[level].tolist())
            mem_mask = torch.zeros(mem_mask_shape).to(reference_points) < 1
            # mem_mask = torch.zeros(mem_mask_shape) < 1
            for bs in range(reference_points.shape[1]):
                ref = reference_points[:,bs,:]
                h,w = src_size[bs][level]
                ref = box_ops.box_cxcywh_to_xyxy(ref)
...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=300, num_encoder_layers=6,
        ...
        self.decoder = TransformerCascadeDecoder(decoder_layer, num_decoder_layers, decoder_norm,
                                          return_intermediate=return_intermediate_dec,
                                          d_model=d_model, query_dim=query_dim, keep_query_pos=keep_query_pos, query_scale_type=query_scale_type,
                                          modulate_hw_attn=modulate_hw_attn,
                                          num_feature_levels=num_feature_levels,
                                          cascade_attn=cascade_attn)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds,tgt,attn_mask=None,src_size=None,orig_size=None):
        """
        Input:
        ...
        hs, references = self.decoder(tgt, memory,tgt_mask=attn_mask, memory_key_padding_mask=mask_flatten,
                        ...
                        src_size=src_size,orig_size=orig_size)
...
class TransformerCascadeDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
                ...
                src_size = None, # for mem_mask
                orig_size = None, # for mem_mask_large
                ):
        output = tgt
        ...
        for layer_id, layer in enumerate(self.layers):
            ...
            if self.cascade_attn:
                memory_mask = gen_memory_mask(reference_points,src_size,spatial_shapes,layer.nhead)
            else:
                memory_mask = [None] * 4
            
            output,weight_cross = layer(output, memory, tgt_mask=tgt_mask,
...
def build_transformer(args):
    return Transformer(
        ...
        cascade_attn=args.cascade_attn,
    )
